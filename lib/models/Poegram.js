...
schema.methods.formattedRes = async function(format, req, res) {
  const formatted = await this.formatted(format, req);
...
schema.methods.formatted = async function(format = 'json', req = '') {
  if(format === 'json') return this.toJSON();
  if(format === 'text') return `${this.poemId.lines}\n—${this.poemId.author} from "${this.poemId.title}"`;
  if(format === 'imagepath') return req.protocol + '://' + req.get('host') + `/poegrams/${this._id}-image.png`;
  if(format === 'image') {
